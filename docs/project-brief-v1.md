# Archived: Original Project Brief (Phase 0)

*This is the original project brief from Phase 0. See `project-brief.md` for the current version.*
...
- Pilot grid runs (L={64,128,192,256} x T={0.50,1.00,1.50} x S=42): 12 runs complete
...
**Deliverables:**
- Crossover T-densification: L={64,128,192} x T={0.60,0.70,0.80,0.90}: 12 runs complete
- L-densification at T=0.50: L={160,176,208,224} x S={42,123,7}: 15 runs complete
- Seed replication at T=0.50: L={64,128,192} x S={42,123,7}: 9 runs complete
...
- Transfer functions: T->C and T->H curves at each L — partially available from existing data
...
- Does L=256 extend collapse further into T? (In-progress: L=256 x T={0.60-0.90} sweep running)
