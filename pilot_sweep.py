"""Batch runner for pilot sweep grid conditions.

Runs all pending pilot grid conditions sequentially with crash isolation,
automatic resume, and progress reporting. Safe to re-run after interrupts.

Usage:
    python pilot_sweep.py
    python pilot_sweep.py --dry-run
"""
...
# Grid definition: (L, T, seed) in priority order.
# New context lengths first (L=128, L=192), then existing (fill gaps).
GRID: list[tuple[int, float, int]] = []
for L in [128, 192, 64, 256]:
...
def is_complete(L: int, T: float, seed: int) -> bool:
    """Check if a run's parquet exists with the expected experiment token count."""
...
def run_condition(L: int, T: float, seed: int) -> subprocess.CompletedProcess:
    """Run a single generation condition as a subprocess."""
    cmd = [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run pilot sweep grid")
    ...
    if not args.dry_run:
        log_file = log_dir / f"sweep_log_{timestamp}.txt"
    ...
    log.info("Sweep: %d conditions pending, %d already done",
    ...
    # Final summary
    log.info("=" * 60)
    log.info("Sweep complete. %d done, %d failed, %d skipped (already done).",
