"""Batch runner for L-densification around the L=192 anomaly at T=0.50.

Maps the non-monotonic compressibility boundary between L=128 and L=256.
Includes L=192 S=7 (in-progress from seed_sweep) so resume picks it up.

Usage:
    python ldense_sweep.py
    python ldense_sweep.py --dry-run
"""
...
# L values: include 192 (for S=7 resume) + new densification points
L_VALUES = [192, 160, 176, 208, 224]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run L-densification sweep at T=0.50")
    ...
    if not args.dry_run:
        log_file = log_dir / f"ldense_sweep_log_{timestamp}.txt"
    ...
    log.info("L-densification sweep: %d conditions pending, %d already done",
    ...
    log.info("=" * 60)
    log.info("L-densification sweep complete. %d done, %d failed, %d skipped (already done).",
