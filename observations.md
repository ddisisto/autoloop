...
**L=8 basin taxonomy: 30 types, two dominant.** Systematic survey (3 seeds × 100k steps, 327 captures) with HDBSCAN clustering on PCA-reduced embeddings yields 30 basin types + 69 noise (21%). Two attractors dominate: zeros/numbers (64 captures, all seeds) and decimal loops (49 captures, all seeds), together 35% of all captures. Remaining types include Python code loops, self-referential sentence fragments ("the sentence is incomplete"), medical/health lists, LaTeX loops, character-level loops, institutional text, and CJK text. 9/30 types are universal (all 3 seeds), 12 appear in 2 seeds, 9 are seed-specific. Within-basin deepening is confirmed: 71% of consecutive same-cluster recaptures go deeper (lower entropy). Discovery is not saturating — every seed finds novel types near the end of its run (last novel at 85-99% through). 100k steps per seed does not exhaust L=8's repertoire.
...
| 2026-03-19 | [observations-2026-03-19.md](docs/observations-2026-03-19.md) | L=8 basin taxonomy complete: 30 clusters from 327 captures. Two dominant (zeros 64, decimal loops 49). Within-basin deepening confirmed (71%). Discovery not saturating (novel types at 85-99% through runs). 9 universal basins, 9 seed-specific. No grab-bags after centroid merge. |
