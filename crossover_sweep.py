"""Batch runner for crossover region T-densification.

Maps the collapse-to-rich transition at T={0.60, 0.75, 0.90} × L={64, 128, 192}.
Same structure as pilot_sweep.py: idempotent, crash-resilient, file logging.

Usage:
    python crossover_sweep.py
    python crossover_sweep.py --dry-run
"""
...
# Grid: T-densification in the crossover region, seed=42 only.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run crossover T-densification sweep")
    ...
    if not args.dry_run:
        log_file = log_dir / f"crossover_sweep_log_{timestamp}.txt"
    ...
    log.info("Crossover sweep: %d conditions pending, %d already done",
    ...
    log.info("=" * 60)
    log.info("Crossover sweep complete. %d done, %d failed, %d skipped (already done).",
